...
public class ObjectSelect : MonoBehaviour {
	...
	void Update() {
		...
		if (isMouseDrag && target != null) {
			...
			Vector3 screenPointOffset = currentScreenSpace - startingPoint;

			// Set the targets position to the mouse pointer / finger position, but switching the y axis with the z axis.
			Vector3 inverse = target.transform.InverseTransformPoint(currentPosition);

			target.transform.Translate(-screenPointOffset.x / 75, 0, -screenPointOffset.y / 50);
			//Quaternion localForward = target.transform.localRotation;
			//Debug.Log(localForward.ToString());
