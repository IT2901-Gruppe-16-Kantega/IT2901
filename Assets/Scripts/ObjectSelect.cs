...
public class ObjectSelect : MonoBehaviour {
	...
	[SerializeField]
	private Button _markTargetButton;

	private int _mouseClicks;
	private float _mouseTimer;

	[SerializeField]
	private Button _resetSignButton;
	...
	private void Update() {
		...
		if (Input.GetMouseButtonDown(0))
			if (_eventSystem.currentSelectedGameObject == null && !_isOverInfoBox) {
				...
				if (_targetPlate != null) {
					_targetPole = _targetPlate.GetComponent<RoadObjectManager>()
						.SignPost;
					IsDragging = true;
		...
		_startingPoint = Input.mousePosition;
		if (!rom.Objekt.geometri.egengeometri)
			if (_targetPole != null)
	...
	public void ZoomChange() {
		if (!_targetPlate) // In case this function is called when no target is selected
			return;
		_changeViewButton.interactable = !ZoomedOnObject;
		if (!ZoomedOnObject) {
			// Zoom out
			...
			Camera.main.transform.LookAt(_targetPlate.transform.position);
			float distance =
				new Vector3(Camera.main.transform.position.x - _targetPlate.transform.position.x, 0,
					Camera.main.transform.position.z - _targetPlate.transform.position.z).magnitude;
			// Magic number
	...
	public void MarkTarget() {
		...
		if (_targetPlate != null)
			_targetPole.GetComponent<SignPlateAdder>()
				.MarkPlates(rom.SomethingIsWrong);

		// Update objectText
		ObjectText.text =
			"ID: " + rom.Objekt.id + "\n" +
			"Egengeometri: " + rom.Objekt.geometri.egengeometri + "\n" +
			"Skiltnummer: " + rom.Objekt.egenskaper.Find(egenskap => egenskap.id == 5530)
				.verdi + "\n" +
			"Manuelt flyttet: " + rom.HasBeenMoved + "\n" +
			"Markert som feil: " + rom.SomethingIsWrong + "\n" +
			"Avstand flyttet: " + string.Format("{0:F2}m", rom.DeltaDistance) + "\n" +
