...
public class MainMenuScript : MonoBehaviour {
	...
	[SerializeField]
	private Button _localDataButton;
	...
	[SerializeField]
	private Button _rnDataButton;

	private void Start() {
		_rnDataButton.onClick.AddListener(delegate {
			GoToMain(false);
		});
		_localDataButton.onClick.AddListener(delegate {
			GoToMain(true);
		});
	}
	...
	public void GoToMain(bool localData) {
		...
		// 1 is true, 0 is false. PlayerPrefs does not have a SetBool function.
	...
	private IEnumerator AnimateOptions() {
		...
		if (_isOptionShown) {
			...
			while (_optionsScreen.anchoredPosition.x <= 719) {
				...
				_optionsScreen.anchoredPosition = Vector2.Lerp(_optionsScreen.anchoredPosition, _optionScreenStartPosition,
					AnimationDampening);
			...
			while (_optionsScreen.anchoredPosition.x >= 1f) {
				...
				_optionsScreen.anchoredPosition = Vector2.Lerp(_optionsScreen.anchoredPosition, _optionScreenEndPosition,
					AnimationDampening);
				yield return new WaitForEndOfFrame();
			}
